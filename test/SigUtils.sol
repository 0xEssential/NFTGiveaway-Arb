...
contract SigUtils is Test {
    ...
    function buildRequest(bytes memory selector, address from, address authorizer)
        external
        returns (IForwardRequest.ERC721ForwardRequest memory req)
    {
    ...
    function signRequest(uint256 pk, IForwardRequest.ERC721ForwardRequest memory request) external view returns (bytes memory) {
    ...
    function mockOwnershipSig(IForwardRequest.ERC721ForwardRequest memory request) external view returns (bytes memory) {
